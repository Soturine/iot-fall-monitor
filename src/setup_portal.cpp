...
void SetupPortal::appendPortalScript(String& html) const {
  ...
  html += "const validateBackendUrl=(value)=>{let parsed;try{parsed=new URL(String(value||'').trim());}catch{return 'Informe uma backendApiBaseUrl HTTP/HTTPS valida.';}if(!/^https?:$/.test(parsed.protocol)){return 'Use http:// ou https:// na backendApiBaseUrl.';}if(isLoopbackHost(parsed.hostname)){return 'Nao use localhost/127.0.0.1/::1 no ESP32.';}return '';};";
  html += "const applyPayload=(payload)=>{if(!payload||typeof payload!=='object'){throw new Error('O payload do QR precisa ser um JSON com backendApiBaseUrl e pairingCode.');}const backendApiBaseUrl=String(payload.backendApiBaseUrl||'').trim();const pairingCode=String(payload.pairingCode||'').trim();const urlError=validateBackendUrl(backendApiBaseUrl);if(urlError){throw new Error(urlError);}if(!pairingCode){throw new Error('O pairingCode do QR veio vazio.');}pairingBackendField.value=backendApiBaseUrl;generalBackendField.value=backendApiBaseUrl;pairingCodeField.value=pairingCode.toUpperCase();setStatus('Dados do QR importados. Revise e clique em Parear agora.','success');};";
